...
export function buildStaticCommands(h: PaletteHandlers): PaletteCommand[] {
  return [
    ...
    },
    {
      id: 'act-import',
      label: 'Import backup',
      hint: 'Single graph or multi-workspace JSON',
      section: 'Actions',
      keywords: [
        'restore', 'load', 'file', 'import', 'json',
        'workspaces', 'bundle', 'multi-workspace', 'backup',
      ],
      icon: Upload,
      run: () => h.importJSON(),
    },
    {
      ...
      hint: 'JSON graph for this workspace only',
      section: 'Actions',
      keywords: ['download', 'save', 'backup', 'json'],
      icon: Download,
      run: () => h.exportJSON(),
    },
    {
